...
describe('#415 slice 2: CSS ships the share-toggle styling', () => {
  ...
  test('mobile <=720px wraps the share checkbox onto its own row above the buttons', () => {
    // On phones Collapse + share label + Save + Clear all in one row
    // overlapped. The mobile media block enables flex-wrap and pins the
    // share toggle to flex-basis:100% + order:-1 so it lives on its own
    // line above the three buttons.
    const mobileIdx = reportsCss.indexOf('@media (max-width: 720px)');
    expect(mobileIdx).toBeGreaterThan(0);
    const mobileSlice = reportsCss.slice(mobileIdx);
    expect(mobileSlice).toMatch(/\.filter-panel-footer--stack[\s\S]*?flex-wrap:\s*wrap/);
    expect(mobileSlice).toMatch(/\.filter-share-toggle[\s\S]*?flex-basis:\s*100%[\s\S]*?order:\s*-1/);
  });
});
