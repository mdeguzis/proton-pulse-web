...
async function populateScoringTooltip(el) {
  ...
  container.innerHTML = `
    ...
    <h4 style="margin:0 0 6px">9. Playtime Confidence Bonus</h4>
    Reports that declare actual playtime are more credible. The bonus is applied inside the GPU/OS/kernel multiplier so it scales with system match.<br>
    ...
    When submitting a report from the plugin, the duration field is pre-filled from your actual playtime. Time logged with the <em>specific active config</em> is weighted 1.25x over your total Steam playtime, since hours with that exact setup are more relevant than lifetime hours across all configs.<br><br>
...
async function fetchNativeReports(appId) {
  try {
    const r = await fetch(
      `${SB_URL}/user_configs?app_id=eq.${appId}&select=id,client_id,app_id,title,cpu,gpu,gpu_driver,gpu_vendor,ram,os,kernel,proton_version,rating,duration,notes,vram_mb,form_responses,config_key,created_at,source&order=created_at.desc`,
      { headers: { apikey: SB_KEY, Authorization: `Bearer ${SB_KEY}` } }
    );
    if (!r.ok) return [];
    const rows = await r.json();
...
function renderCard(r, votes, userVotes = {}, configPlaytimeTotals = []) {
  ...
  return `
    ...
    <div class="card-summary">
      ...
      ${fmtDuration(r.duration) ? `<div class="row"><span class="label">Steam playtime</span><span>${fmtDuration(r.duration)}</span></div>` : ''}
