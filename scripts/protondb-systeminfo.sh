#!/bin/bash
# Proton Pulse system-info tool. Prints your hardware in Steam System
# Information format so you can paste it into "Parse from Steam info" on
# proton-pulse.com. Run it, then copy the output.
#
# Adapted from mdeguzis/SteamOS-Tools (utilities/protondb-systeminfo-tool.sh),
# originally yanked from ChimeraOS. Kept in this repo so the site can offer a
# fixed, versioned copy.
export LC_ALL=C
...
# Sound card (pulsemixer is optional and missing on many systems; fall back to
# pactl, then to Unknown so the script never errors out):
if command -v pulsemixer >/dev/null 2>&1; then
    AUDIO_DEVICE=$(pulsemixer --list-sinks | grep Default | cut -d',' -f2 | sed 's/ Name: //')
elif command -v pactl >/dev/null 2>&1; then
    AUDIO_DEVICE=$(pactl info 2>/dev/null | grep 'Default Sink:' | cut -d':' -f2 | sed 's/^ *//')
fi
[ -z "${AUDIO_DEVICE}" ] && AUDIO_DEVICE="Unknown"
...
for BLOCK_DEVICE in $(lsblk --nodeps --output name,tran,rota --noheadings --exclude 7); do
    ...
    if [ "${TRANSPORT}" == "usb" ]; then
        ...
        # String compare, not -eq: ROTATIONAL can be empty for some devices,
        # which made "[ -eq 1 ]" throw "integer expected". Only count when the
        # rotational flag is explicitly reported.
