...
// endpoint is a whitelisted key ('appdetails' | 'deck'); appId must be numeric.
// Returns the proxy payload { ok, endpoint, app_id, url, status, data } or
// { ok:false, error }.
export async function exploreSteam(endpoint, appId) {
  try {
    const res = await fetch(STEAM_EXPLORE_ENDPOINT(), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ endpoint: String(endpoint), app_id: String(appId) }),
      cache: 'no-store',
    });
